...
int main(void) {
	...
	while (1) {
		...
		if (direction == CLOCKWISE && counter > 9) {
			counter = 0;
		} else if (direction == COUNTERCLOCKWISE && counter < 0) {
			counter = 9;
		}

		rescaled_pot = 50 + ((raw_pot * 150) / 4095);
		...
		case CLOCKWISE: {
			...
			if (HAL_GetTick() > (last_update + rescaled_pot)) {
				sct_value(counter);  // Display the current value
